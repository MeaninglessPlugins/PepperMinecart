...
| 数据安全 | 方块显示随实体 NBT 自动持久化；容器内容存实体 PDC；矿车被破坏时掉落方块与内容（特殊矿车按原版设计掉落合并物品与内容），不丢失、不复制 |
| 交互体验 | 交互冷却防连点、放置/取下音效开关、容器类矿车独立取下策略（PICKUP/DROP/FORBIDDEN） |
| 扩展性 | 统一 `CartTypeHandler` 扩展接口，第三方插件可注册全新矿车类型（见下文"扩展指南"） |

## 安装

1. 将 `PepperMinecart-1.0.0-all.jar` 放入服务器 `plugins/` 目录（不带 `-all` 的是瘦包，无内置 bstats，部署会 `NoClassDefFoundError`）
2. 重启服务器（或 `/reload`）
...
- 矿车被破坏：普通方块矿车掉落方块物品与容器内容；原版特殊矿车由原版按设计掉落合并物品（如 `Minecart with Chest`）与内容物，不丢失。
...
**Q：矿车被炸了数据会丢吗？**
A：不会。普通方块矿车会掉落方块物品与容器内容；原版特殊矿车由原版按设计掉落合并物品（如 `Minecart with Chest`）与内容物；显示方块随实体消亡，无残留。

**Q：为什么会"复制"方块？**
A：插件在矿车死亡时清空实体数据并移除显示，所有路径统一走引擎默认行为；特殊矿车死亡路径直接采用原版掉落，插件不再额外掉落，避免复制。

## 构建

```bash
./gradlew build        # 产物: build/libs/PepperMinecart-1.0.0-all.jar
...
- 熔炉矿车取下与原版一致，为普通熔炉物品（不携带燃料）；放置带内容的熔炉时，输入/燃料/成品会掉落到矿车位置（熔炉矿车实体没有库存，无法承载）。
- 命令方块矿车放置/取下时保留已设置的命令（原版只有普通命令矿车一种实体，连锁/循环类型按普通命令矿车处理）。
- 木桶开合动画（完整版）需发包伪造方块，暂以音效替代。
- 监听 `EntityRemoveEvent`（Paper 1.17.1+ 提供）以覆盖插件 `remove()` 等全部矿车移除路径。
- 箱子/漏斗矿车取下时返还"带库存的容器物品"（内容组装进物品 NBT 随物品回收）；特殊矿车被原版破坏时按原版设计掉落合并物品（如 `Minecart with Chest`）与内容物，插件不再额外掉落，防止复制。
